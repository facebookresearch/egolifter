...
DATA_ROOT=/source_1a/data/qgu/aria_processed/pilot_2024_v1/
OUTPUT_ROOT=/source_1a/data/qgu/output/pilot_2024_v1/
...
submit_jobs() {
    ...
    for folder in "${folders[@]}"; do
        if [ -n "$dependency" ]; then
            jobid=$(sbatch --dependency=afterok:$dependency --job-name="gs-$folder" --parsable ./scripts/run_scene.bash "$DATA_ROOT" "$folder" "$OUTPUT_ROOT")
        else
            jobid=$(sbatch --job-name="gs-$folder" --parsable ./scripts/run_scene.bash "$DATA_ROOT" "$folder" "$OUTPUT_ROOT")
        fi
        jobids+=($jobid)
    done
