...
Compared to process_adt_3dgs.py, this additionally consider the online calibration files. 
'''
...
class ProcessAriaDigitalTwin:
    ...
    def main(self) -> None:
        ...
        for selected_device_number in range(len(all_device_serials)):
            ...
            # Use the factory claibration for vignette and mask images
            src_calib_factory = device_calib.get_camera_calib(sensor_name)
            ...
            # Load the online calibration
            online_calibration_file = os.path.join(data_folder, "mps", "slam", "online_calibration.jsonl")
            assert os.path.exists(online_calibration_file), f"cannot find online calibration file {online_calibration_file}"
            online_camera_calibs = mps.read_online_calibration(str(online_calibration_file))
            ...
            # The factory calibration may be off
            transform_device_cam_factory = gt_provider.get_aria_transform_device_camera(stream_id)

            # Get the valid mask and vignette
            # TODO: what distortion params to use for undistort vignette and valid mask
            ...
            rectified_vignette_rgb = calibration.distort_by_calibration(vignette_rgb, dst_calib, src_calib_factory)
            ...
            rectified_mask_rgb = calibration.distort_by_calibration(mask_rgb, dst_calib, src_calib_factory)
            ...
            for idx, timestamp_ns in enumerate(tqdm(img_timestamps_ns)):
                ...
                # Get the distoration parameters and device_cam transformation using online calibration
                nearest_calib_idx = bisection_timestamp_search(online_camera_calibs, timestamp_ns_pose_search)
                if nearest_calib_idx is None:
                    print(f"Cannot find online calibration for timestamp {timestamp_ns_pose_search}")
                    continue
                ...
                camera_calib_time_diff_ns = abs(calib_time_ns - timestamp_ns_pose_search)
                if camera_calib_time_diff_ns > 3_000_000: # 3 ms
                    print(f"Online calibration timestamp {calib_time_ns} is too far from pose timestamp {timestamp_ns_pose_search} (diff: {camera_calib_time_diff_ns} ns)")
                    continue
                
                # find the one that is RGB camera
                camera_calib = None 
                for calib in camera_calibration.camera_calibs: 
                    if calib.get_label() == "camera-rgb": 
                        camera_calib = calib 
                        break 
                assert camera_calib is not None, "Did not find camera-rgb calibration in online calibrations!"
                transform_device_cam = camera_calib.get_transform_device_camera()
                ...
                rectified_image = calibration.distort_by_calibration(image, dst_calib, camera_calib)

                # Segmentation
                segmentation_data = segmentation_with_dt.data().to_numpy_array()
                rectified_segmentation_data = calibration.distort_label_by_calibration(segmentation_data, dst_calib, camera_calib)

                # Segmentation for visualization
                segmentation_viz = segmentation_with_dt.data().get_visualizable().to_numpy_array()
                rectified_segmentation_viz = calibration.distort_label_by_calibration(segmentation_viz, dst_calib, camera_calib)
