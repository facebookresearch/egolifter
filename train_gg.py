...
from gaussian_grouping.gaussian_renderer import render
from gaussian_grouping.scene import Scene, GaussianModel
from gaussian_grouping.utils.general_utils import safe_state
from gaussian_grouping.utils.image_utils import psnr
from gaussian_grouping.arguments import ModelParams, PipelineParams, OptimizationParams
from gaussian_grouping.utils.loss_utils import l1_loss, ssim, loss_cls_3d
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_wandb):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        image_raw, viewspace_point_tensor, visibility_filter, radii, objects = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"], render_pkg["render_object"]
        ...
        # Mask the loss with the valid pixel mask
        if scene.valid_mask_by_name is not None:
            loss_obj = loss_obj * scene.valid_mask_by_name['rgb'][1.0].squeeze(0)
        ...
        # post-processing: apply valid_mask and vignette
        image, gt_image = scene.postProcess(image_raw, gt_image, "rgb")
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    wandb.log({
                        "scene/n_gaussians": gaussians.get_xyz.shape[0]
                    })
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    image_raw = torch.clamp(renderFunc(viewpoint, scene.gaussians, *renderArgs)["render"], 0.0, 1.0)
                    
                    # gt_image = torch.clamp(viewpoint.original_image.to("cuda"), 0.0, 1.0)
                    gt_image = viewpoint.get_image().cuda().clamp(0.0, 1.0)
                    ...
                    if use_wandb:
                        if idx < 5:
                            ...
                            wandb.log({config['name'] + "_view_{}/render_raw".format(viewpoint.image_name): [wandb.Image(image_raw)]})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb_project", type=str, default="gg_adt", help="Wandb project name")
