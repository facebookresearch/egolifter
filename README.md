...
* Set up the environment. Note that EgoLifter is using a separate implementation of `diff-gaussian-rasterization` and thus it requires creation of a new python environment. 

```bash
conda create --name egolifter_gg --clone egolifter
conda activate egolifter_gg

# TODO: Again, change the below to the actual path where you installed the CUDA toolkit
export PATH=/usr/local/cuda-12.3/bin/:${PATH}
export CUDA_HOME=/usr/local/cuda-12.3/

# Use the existing code for Gaussian Grouping in this repo.
cd gaussian_grouping

# Note that EgoLifter and Gaussian Grouping are using different implementations of `diff-gaussian-rasterization`.
pip install submodules/diff-gaussian-rasterization

# Use the existing DEVA codebase. Do not clone the DEVA codebase again.
...
* Preprocessing. Again, run the following inside the `gaussian_grouping` folder

```bash
bash script/prepare_pseudo_label_adt.sh ${ADT_PROCESSED_ROOT} ${SCENE_NAME}
```

### Training 

Run the following command to train a Gaussian Grouping model on ADT dataset and DEVA segmentation results

```bash
cd .. # Go to the `egolifter` root directory

python train_gg.py \
    -s ${ADT_PROCESSED_ROOT}/${SCENE_NAME} \
    -r 1  \
    -m output/adt_gg/${SCENE_NAME} \
    --config_file gaussian_grouping/config/gaussian_dataset/train.json \
    --data_device cpu \
    --use_wandb
```
...
```bash
# Here we still need the EgoLifter checkpoint to load the relevant scene information
# But the model type and weight will be overwritten by the Gaussian Grouping trained checkpoint
EGOLIFTER_CKPT_ROOT=./output/adt
GG_CKPT_ROOT=./output/adt_gg

# 2D segmentation evaluation, in-view setting
python eval_query_2dseg_gg.py \
    --ckpt_folder ${EGOLIFTER_CKPT_ROOT}/${SCENE_NAME}/unc_2d_unet_egolifter \
    --source_path ${ADT_PROCESSED_ROOT}/${SCENE_NAME} \
    --gg_ckpt_folder ${GG_CKPT_ROOT}/${SCENE_NAME} \
    --query_type inview \
    --threshold_mode gt

# 2D segmentation evaluation, cross-view setting
python eval_query_2dseg_gg.py \
    --ckpt_folder ${EGOLIFTER_CKPT_ROOT}/${SCENE_NAME}/unc_2d_unet_egolifter \
    --source_path ${ADT_PROCESSED_ROOT}/${SCENE_NAME} \
    --gg_ckpt_folder ${GG_CKPT_ROOT}/${SCENE_NAME} \
...
python eval_lightning.py \
    model_path=${EGOLIFTER_CKPT_ROOT}/${SCENE_NAME}/unc_2d_unet_egolifter \
    source_path=${ADT_PROCESSED_ROOT}/${SCENE_NAME} \
    gg_ckpt_folder=${GG_CKPT_ROOT}/${SCENE_NAME} \
    eval_on_gg=True
...
<details>
<summary>Run EgoLifter on EgoExo4D dataset</summary>

<!-- First obtain the access to EgoExo4D dataset on the [official website](https://ego-exo4d-data.org/#intro). Set up the ego4d codebase following the instructions [here](https://github.com/facebookresearch/Ego4d?tab=readme-ov-file#setup).  -->
